...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (auxpow != NULL) {
        ...
        // print to log
        LogPrintf("LiteGoldCoinMiner: AUX proof-of-work found; our hash: %s ; parent hash: %s ; target: %s\n",
               hash.GetHex().c_str(),
    ...
    else
    {
        ...
        // print to log
        LogPrintf("LiteGoldCoinMiner: proof-of-work found; hash: %s ; target: %s\n", hash.GetHex().c_str(), hashTarget.GetHex().c_str());
    }
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("LiteGoldCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LiteGoldCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LiteGoldCoinMiner(CWallet *pwallet)
{
    LogPrintf("LiteGoldCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("litegoldcoin-miner");
    ...
    try { while (true) {
        ...
        LogPrintf("Running LiteGoldCoinMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("LiteGoldCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet, int nThreads)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LiteGoldCoinMiner, pwallet));
}
