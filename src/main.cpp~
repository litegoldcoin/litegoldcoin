...
// Copyright (c) 2013-2014 The LiteGoldCoin developers
...
#if defined(NDEBUG)
# error "LiteGoldCoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "LiteGoldCoin Signed Message:\n";
...
//TODO: this isn't identical to litegoldcoin reference client.
...
static const int64_t nTargetTimespan = 10 * 30; // LiteGoldCoin: every 5min
static const int64_t nTargetTimespanNEW = 30 ; // LiteGoldCoin: every 30sec
static const int64_t nTargetSpacing = 30; // LiteGoldCoin: 30sec 
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // LiteGoldCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
void ThreadScriptCheck() {
    RenameThread("litegoldcoin-scriptch");
...
bool ConnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // BIP16 was always active in LiteGoldCoin
...
bool AcceptBlockHeader(CBlockHeader& block, CValidationState& state, CBlockIndex** ppindex)
{
    ...
    if (hash != Params().HashGenesisBlock()) {
        ...
        // LiteGoldCoin: reject v2 and v1 blocks at the same time, only check once
        if (block.GetBaseVersion() < 3) {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp)
{
    ...
    // LiteGoldCoin: reject ONLY if block.nVersion=3 has a supermajority because CBlockIndex::IsSuperMajority
