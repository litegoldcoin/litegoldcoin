...
*Setup instructions for a gitian build of LiteGoldCoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the LiteGoldCoin
...
- [Building LiteGoldCoin](#building-litegoldcoin)
...
Clone the git repositories for litegoldcoin and gitian,

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/litegoldcoin/litegoldcoin
```
...
In [doc/release-process.md](release-process.md) in the litegoldcoin repository under 'Fetch and build inputs'.
...
Building LiteGoldCoin
----------------

To build LiteGoldCoin (for Linux and/or Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the litegoldcoin repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/litegoldcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/litegoldcoin/litegoldcoin
...
For example:
```bash
URL=https://github.com/langerhans/litegoldcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit litegoldcoin=${COMMIT} --url litegoldcoin=${URL} ../litegoldcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit litegoldcoin=${COMMIT} --url litegoldcoin=${URL} ../litegoldcoin/contrib/gitian-descriptors/gitian-win.yml
```
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/litegoldcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/litegoldcoin-build.assert
```
...
[litegoldcoin/gitian.sigs](https://github.com/litegoldcoin/gitian.sigs/) repository, or
